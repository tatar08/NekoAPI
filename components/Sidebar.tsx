...
export default function Sidebar({ activeView, setActiveView }: SidebarProps) {
  ...
  const pinnedRequests = collections.flatMap(col => 
    (col.requests || []).filter(r => r.isPinned).map(req => ({ req, colId: col.id }))
  );
            ...
            {col.requests.map((req: RequestModel) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`flex items-center gap-1.5 transition ${req.isPinned ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        togglePinRequest(req.id, col.id);
                      }}
                      className={`transition duration-150 p-0.5 cursor-pointer text-xs ${
                        req.isPinned
  ...
  return (
    <div className="flex flex-col h-full text-xs text-gray-300 select-none">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 flex flex-col gap-4">
        {/* Pinned / Starred Requests Section */}
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-1.5 px-1 py-1 border-b border-white/[0.02]">
              <span className="text-[10px] font-bold text-amber-400 uppercase tracking-wider flex items-center gap-1">
                <span>★</span>
                <span>Starred Requests</span>
              </span>
            </div>
            <div className="flex flex-col border border-white/[0.03] bg-[#0c0d14]/30 rounded-xl overflow-hidden shadow-sm">
              {pinnedRequests.map(({ req, colId }) => {
                const isTabActive = activeTabId === req.id;
                return (
                  <div
                    key={req.id}
                    className={`flex items-center justify-between px-3 py-2.5 border-t border-white/[0.02] first:border-t-0 cursor-pointer group transition duration-150 select-none ${
                      isTabActive ? 'bg-violet-950/20' : 'hover:bg-white/[0.02]'
                    }`}
                    style={{ borderLeft: isTabActive ? '2px solid var(--accent-primary)' : '2px solid transparent' }}
                    onClick={() => {
                  ...
                  >
                    <div className="flex items-center gap-2.5 overflow-hidden flex-1">
                      ...
                      <span className="truncate text-xs font-semibold text-gray-300">{req.name}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          togglePinRequest(req.id, colId);
                        }}
                        className="text-amber-400 hover:text-amber-300 p-1 cursor-pointer transition duration-150 text-xs"
                        title="Unstar Request"
                      >
                        ★
                      </button>
                    </div>
